...
| `result-mistakes.css` L1–370+ | Battle result modal (`monsterResultShown`) | `.result-modal-glass`, `.result-stat-*`, `.result-action-*`, spirit hero | Low–Med | `result-mistakes.css` | — | 模板 `index.html` L3884–3970；邏輯見 `game-js-map.md` §結算畫面、`result-display-manager.js` |
| `result-mistakes.css` + `index.html` | Stage records + mistakes modals | `.stage-record-*`（L3198+ / L4618+）；`.mistakes-*`（~L4936+） | Low–Med | `result-mistakes.css` | — | `stageRecordRows` 來自 manager；錯題資料 `mistakes` 在 `game.js` #30 |
...
| `home.css` ~L257–810 | Save slot panel (home overlay) | `.save-slot-overlay` … `.save-slot-delete-*`；資料來自 `saveSlotCards` | Low–Med | `home.css` | — | 與 `game-js-map.md` §首頁存檔卡 對照；`.save-slot-subtitle` 目前無 `index.html` 綁定（樣式預留） |
