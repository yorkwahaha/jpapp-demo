...
| L6761-L6945 | Stage confirm modal | Stage confirm panel, mentor entry button, stage best record display | Medium | `map.css` / `mentor.css` | No | 模板 `index.html` L2548–2633；最佳紀錄顯示 `getStageBest*`（`result-display-manager.js`）；開戰按鈕綁 `confirmAndStartBattle`（**勿當 display 改**）。見 `game-js-map.md` §地圖顯示層 |
