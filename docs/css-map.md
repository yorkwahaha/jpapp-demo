...
| `result-mistakes.css` 檔尾 | 答錯對比教學 + 出題本錯題置頂/答案行 | `.battle-wrong-tip`（`index.html` L3671）；`.mistakes-sort-btn`（~L4901）；`.stage-log-ans*`（~L4948） | Low | `result-mistakes.css` | — | 顯示條件：`wrongAnswerTip` / `stageLogMistakesFirst`（`game.js` ~7746–7775）；對比文案 `particle-contrasts.v1.json` |
