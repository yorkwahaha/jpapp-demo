...
| `statsData` computed | ~#11 區（`srsAvailable` 附近） | 呼叫 `window.JPAPPStatsDisplay.computeStats()`；依賴 `SKILLS`, `SPIRITS`, `skillMastery`, `stageBestRecords`, `bestGrades`, `mistakes`；第5批B 加入 `spirits` 參數與 `familyStats` 回傳 |
