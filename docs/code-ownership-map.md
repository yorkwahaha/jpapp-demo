...
| before game | `stats-display.js` | `JPAPPStatsDisplay` | 學習統計彙整（第4批A + 第5批B）：`computeStats({ skills, spirits, skillMastery, stageBestRecords, bestGrades, mistakes })` → `{ particleStats, overallMastery, weakest3, clearedCount, sRankCount, mistakeCount, familyStats }`；內部 `computeFamilyStats()` 以 spirit.family 分組 |
